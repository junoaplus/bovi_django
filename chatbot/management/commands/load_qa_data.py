...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        gpt_samples = [
            ...
            {
                'game_name': '뱅',
                'question': '뱅 게임의 기본 진행 방식은?',
                'answer': '뱅은 서부 테마의 숨겨진 역할 게임입니다. 보안관, 부관, 무법자, 배신자 중 하나의 역할을 맡아 각자의 목표를 달성해야 합니다. 턴마다 카드 2장을 뽑고, 카드를 사용하며, 손에 든 카드 수를 생명력 이하로 조정합니다.'
            }
        ]
        ...
        finetuning_samples = [
            ...
            {
                'game_name': '뱅',
                'question': '뱅에서 거리 계산과 공격 범위는 어떻게 정해지나요?',
                'answer': '뱅에서 거리는 시계방향으로 계산하며, 플레이어 간 최단 거리를 적용합니다. 기본 공격 범위는 1이며, 무기 카드로 범위를 확장할 수 있습니다. 말 카드는 다른 플레이어로부터의 거리를 1 줄여주고, 배럴은 뱅 카드에 대한 방어 기회를 제공합니다. 감옥에 갇힌 플레이어는 하트를 뽑아야 탈출할 수 있습니다.'
            }
        ]
        ...
        for sample in gpt_samples:
            ...
            if created:
                ...
                self.stdout.write(f'⚠️  이미 존재: {gpt_qa.game_name} - {gpt_qa.question[:30]}...')
        ...
        for sample in finetuning_samples:
            ...
            if created:
                ...
                self.stdout.write(f'⚠️  이미 존재: {ft_qa.game_name} - {ft_qa.question[:30]}...')
        ...
        self.stdout.write(f'📊 GPT 룰 QA: {gpt_count}개')
        self.stdout.write(f'📊 파인튜닝 룰 QA: {ft_count}개')
        self.stdout.write(f'📊 총 QA: {gpt_count + ft_count}개')
